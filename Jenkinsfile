...
//                 sh 'docker run my_test1 --browser chrome'
                sh 'docker run --name my_test_97 my_test1 --url ${URL} --executor ${EXECUTOR} --browser ${BROWSER}'

//                 sh 'docker cp my_test_18:/app/allure-result /var/jenkins_home/workspace/test2/allure-report'
                sh 'docker cp my_test_97:/app/allure-result /var/jenkins_home/workspace/test2/allure-results'
                sh 'ls -la'
